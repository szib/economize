class Account < ApplicationRecord
  ...
  def most_expensive_service
    ...
    self.active_subscriptions_ordered_by_price.first
 end
  ...
  def lifetime_spend
    ...
    self.subscriptions.each do |subscription|
      subscription.billing_dates_array.each do |day|
        total_spend = total_spend+subscription.service.monthly_price_on_given_day(day)
      end
    end
    total_spend.round(2)
  end
 ...
 def cancelled_services
   ...
   services_of_non_active_subscriptions = self.subscriptions.select{|subscription| subscription.end_date != nil}.map(&:service).map{|service| service.name}
   services_of_active_subscriptions = self.subscriptions.select{|subscription| subscription.end_date == nil}.map(&:service).map{|service| service.name}
   services_of_non_active_subscriptions.select{|service| !services_of_active_subscriptions.include?(service)}
end
