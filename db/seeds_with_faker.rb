...
Service.all.each do |service|
  ...
  price_record_dates = (1..30).to_a.sample.times.map do
    Faker::Date.between(service_start_date, Date.today)
  end.sort.uniq

  baseprice = (5..100).to_a.sample
  price_record_data = price_record_dates.each_with_index.map do |date, idx|
    {
      effective_from: date,
      monthly_price: baseprice + (baseprice * 0.05 * idx),
...
Account.all.each do |account|
  service_ids = []
  (0..20).to_a.sample.times { service_ids << all_service_id.sample }
  ...
  service_ids.uniq.each do |service_id|
    ...
    end_date = if (1..100).to_a.sample >= 40
                 nil
               else
